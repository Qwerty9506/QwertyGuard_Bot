...
async def manage_group(call: CallbackQuery, state: FSMContext):
    ...
    # Теперь здесь вместо "Перейти в чат" будет название группы
...
# --- ДОБАВЛЕНО: ОБРАБОТЧИК МОДЕРАЦИИ ---
...
# --- НАСТРОЙКИ ИНВАЙТОВ (ОБНОВЛЕНО С КНОПКАМИ ВКЛ/ВЫКЛ) ---
...
async def toggle_invites(call: CallbackQuery, state: FSMContext):
    ...
    if current_val > 0:
        ...
        await db.update_req_invites(group_id, 1) # По умолчанию ставим 1 при включении, если было 0
...
# --- НАСТРОЙКИ АНТИСПАМА (ОБНОВЛЕНО ТЕКСТ) ---
...
async def settings_spam(call: CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        "• **Не прошел проверку** ➔ Бан и удаление спама.\n"
