...
async def group_message_handler(message: Message, bot: Bot):
    ...
    # --- 1. АНТИСПАМ (АВТО-КИК) ---
    # Игнорируем альбомы с медиа (чтобы не банить за несколько фото сразу)
    if not message.media_group_id:
        now = time.time()
        if (chat_id, user_id) in SPAM_CACHE:
            last_time, count = SPAM_CACHE[(chat_id, user_id)]
            if now - last_time < 1.3:  
                count += 1
                SPAM_CACHE[(chat_id, user_id)] = [now, count]
                if count >= 4:  # Увеличили порог до 4, чтобы избежать ложных срабатываний
                    try:
                        await bot.ban_chat_member(chat_id, user_id)
                        await bot.unban_chat_member(chat_id, user_id, only_if_banned=True)
                        user_fullname = html.escape(message.from_user.full_name)
                        user_link = f'<a href="tg://user?id={user_id}">{user_fullname}</a>'
                        await message.answer(f"Спамер {user_link} был кикнут (Авто-модерация)", parse_mode="HTML")
                    except Exception:
                        pass
    ...
    # Формируем HTML ссылки (с экранированием тегов)
    t_full_esc = html.escape(target_fullname)
    a_full_esc = html.escape(message.from_user.full_name)
    user_link = f'<a href="tg://user?id={target_id}">{t_full_esc}</a>'
    admin_link = f'<a href="tg://user?id={user_id}">{a_full_esc}</a>'
